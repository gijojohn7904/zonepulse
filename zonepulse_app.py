...
Get zone-by-zone <i>clarity</i> on DE activity, eliminate idle supply, target churn risks, and drive reliable ops—<b>rain or shine</b>.
""", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    # 1. ZONE-LEVEL HOURLY REPORT
    with st.expander("📊 Zone-Level Hourly Report – View Logic & Tips", expanded=False):
        st.markdown("""
        <div style='background-color:#e7f3fe;padding:16px;border-radius:6px;border:1px solid #b3d8f5;'>
        <b>What this shows:</b> Hour-by-hour zone staffing and utilization.<br>
        <b>How to use:</b> Check for ⚠️ Overstaffed/🔴 Understaffed recommendations.<br>
        - <b>Login Utilization %</b> = (Avg Orders × 25 min) / (Avg Login Minutes) × 100.<br>
        <b>Tip:</b> Aim for as many '✅ Balanced' hours as possible for best efficiency.<br>
        <b>Login Utilization Thresholds:</b><br>
        - <b>Instamart</b>: Overstaffed if Orders/hr &lt; 1.2 &amp; Utilization &lt; 30%, Understaffed if Orders/hr &gt; 2.2 &amp; Utilization &gt; 70%<br>
        - <b>SwiggyFood</b>: Overstaffed if Orders/hr &lt; 1.0 &amp; Utilization &lt; 50%, Understaffed if Orders/hr &gt; 1.2 &amp; Utilization &gt; 57%<br>
        </div>
        """, unsafe_allow_html=True)
    ...
    # 2. RAIN PARTICIPATION ANALYSIS
    with st.expander("🌧️ Rain Participation Analysis – View Logic & Tips", expanded=False):
        st.markdown("""
        <div style='background-color:#e7f3fe;padding:16px;border-radius:6px;border:1px solid #b3d8f5;'>
        <b>What this shows:</b> Which DEs turned up on rain days, and which didn't.<br>
        <b>How to use:</b><br>
        - Focus on "Eligible Actives" (worked ≥6/7 days before rain).<br>
        - "Rain Skippers" = Reliable DEs who skipped only on rain.<br>
        <b>Why:</b> Helps you identify who is rain-proof and who needs follow-up.<br>
        <b>Participation %:</b> (DEs who logged in on rain day) / (Eligible Actives for zone)
        </div>
        """, unsafe_allow_html=True)
    ...
    # 3. ATTRITION RISK DES
    with st.expander("⚠️ Attrition Risk DEs – View Logic & Tips", expanded=False):
        st.markdown("""
        <div style='background-color:#e7f3fe;padding:16px;border-radius:6px;border:1px solid #b3d8f5;'>
        <b>What this shows:</b> DEs who logged in >3 hours but did <2 orders.<br>
        <b>How to use:</b><br>
        - These DEs are at high risk of churn.<br>
        - Action: FM/FC to call and understand issues.<br>
        <b>Pro-tip:</b> High login + low orders = frustration & risk. Move fast.
        </div>
        """, unsafe_allow_html=True)
    ...
    # 4. INDIVIDUAL DE-WISE VIEW
    with st.expander("👤 Individual DE-wise View (Logic & Tips)", expanded=False):
        st.markdown("""
        <div style='background-color:#e7f3fe;padding:16px;border-radius:6px;border:1px solid #b3d8f5;'>
        <b>What this shows:</b> Deep dive on one DE's performance & pattern.<br>
        <b>How to use:</b><br>
        - Check week-on-week login/orders/earnings for improvement or drop.<br>
        - See all login vs order patterns at daily/hourly granularity.<br>
        <b>Action:</b> Spot sudden drop or rising rejections? Coach/engage the DE directly.
        </div>
        """, unsafe_allow_html=True)
    ...
    # 5. NO-SHOW DES
    with st.expander("🤔 No-Show DEs (Logic & Tips)", expanded=False):
        st.markdown("""
        <div style='background-color:#e7f3fe;padding:16px;border-radius:6px;border:1px solid #b3d8f5;'>
        <b>What this shows:</b> DEs who were active before, but not in the current period.<br>
        <b>How to use:</b><br>
        - Quickly spot churned/absent DEs.<br>
        - Follow up to re-engage or update roster.<br>
        <b>Tip:</b> Review no-shows weekly for best retention.
        </div>
        """, unsafe_allow_html=True)
